...
- [ ] 1.1 Create Alembic migration: Add `categorias` table with self-referencing FK
...
- [ ] 1.2 Verify migration is reversible: `alembic downgrade -1` succeeds without errors

- [ ] 1.3 Apply migration to development database: `alembic upgrade head`

## 2. Backend Data Model and Repository

- [ ] 2.1 Create SQLModel `Category` class in `backend/app/models/category.py`
...
- [ ] 2.2 Create `CategoryRepository` in `backend/app/repositories/category_repository.py`
...
- [ ] 2.3 Add `CategoryRepository` to `UnitOfWork` class as `self.categorias`

## 3. Backend Business Logic and Service Layer

- [ ] 3.1 Create `CategoryService` in `backend/app/categorias/service.py`
...
- [ ] 3.2 Write unit tests for `CategoryService` in `backend/tests/test_categorias_service.py`
...
- [ ] 4.1 Create Pydantic schemas in `backend/app/categorias/schemas.py`
...
- [ ] 4.2 Create router in `backend/app/categorias/router.py`
...
- [ ] 4.3 Register router in `backend/app/main.py`: `app.include_router(categorias_router, prefix="/api/v1", tags=["categorias"])`

- [ ] 4.4 Add `require_role(["ADMIN", "STOCK"])` dependency to protected endpoints if not already implemented

## 5. Backend Integration Tests

- [ ] 5.1 Create integration tests in `backend/tests/test_categorias_router.py`
...
- [ ] 5.2 Verify all tests pass: `pytest backend/tests/test_categorias_router.py -v`

## 6. Frontend State Management

- [ ] 6.1 Create `useCategoriesStore` in `frontend/src/features/categories/store/categoriesStore.ts`
...
- [ ] 6.2 Create API client functions in `frontend/src/features/categories/api.ts`
...
- [ ] 7.1 Create `CategoryTree` component in `frontend/src/features/categories/components/CategoryTree.tsx`
...
- [ ] 7.2 Create `CategoryTreeContainer` in `frontend/src/features/categories/components/CategoryTreeContainer.tsx`
  - Use `useCategoriesStore` to fetch tree on mount
  - Handle loading/error states
  - Pass categories to `CategoryTree`

- [ ] 7.3 Add `CategoryTreeContainer` to homepage/navbar for category navigation
  - Display as dropdown or sidebar menu
  - Make categories clickable to filter product listing

## 8. Frontend Tests

- [ ] 8.1 Write unit tests in `frontend/tests/CategoryTree.test.tsx`
  - Test recursive rendering of nested categories
  - Test expand/collapse behavior
  - Test memoization prevents re-renders on unchanged props

- [ ] 8.2 Write integration test in `frontend/tests/categories.integration.test.ts`
...
- [ ] 9.1 Start backend: `cd backend && uvicorn app.main:app --reload`

- [ ] 9.2 Start frontend: `cd frontend && npm run dev`

- [ ] 9.3 Test backend endpoints with Postman/curl:
...
- [ ] 10.1 Run backend linting: `pylint backend/app/categorias/`

- [ ] 10.2 Run backend type checking: `mypy backend/app/categorias/`

- [ ] 10.3 Run frontend linting: `npm run lint`

- [ ] 10.4 Run frontend type checking: `npm run build`

- [ ] 10.5 Add docstrings to all public methods
  - CategoryService methods: describe parameters, return type, exceptions
  - Repository methods: describe SQL behavior

- [ ] 10.6 Update backend API documentation (if using Swagger/OpenAPI): verify /docs endpoint shows category endpoints

## 11. Git Commits and PR Preparation

- [ ] 11.1 Create branch: `git checkout -b change/us-002-categorias`

- [ ] 11.2 Commit database migration: `git commit -m "feat(db): add categorias table with self-referencing FK for hierarchical structure"`

- [ ] 11.3 Commit backend model/repo: `git commit -m "feat(backend): implement Category model and CategoryRepository with CTE queries"`

- [ ] 11.4 Commit backend service/routes: `git commit -m "feat(backend): implement category CRUD endpoints with cycle validation"`

- [ ] 11.5 Commit backend tests: `git commit -m "test(backend): add comprehensive tests for category operations"`

- [ ] 11.6 Commit frontend store/api: `git commit -m "feat(frontend): implement categories store and API client"`

- [ ] 11.7 Commit frontend components: `git commit -m "feat(frontend): add recursive CategoryTree component"`

- [ ] 11.8 Commit frontend tests: `git commit -m "test(frontend): add tests for category tree rendering"`

- [ ] 11.9 Push branch: `git push origin change/us-002-categorias`

- [ ] 11.10 Create pull request with summary from proposal.md

## 12. Final Verification and Archive

- [ ] 12.1 All tests pass: `npm run test:backend && npm run test:frontend`

- [ ] 12.2 No TypeScript errors: `npm run build`

- [ ] 12.3 No Python type errors: `mypy backend/app`

- [ ] 12.4 PR approved and merged to main

- [ ] 12.5 Archive the change: `openspec archive us-002-categorias`
...
- [ ] 12.6 Update FINAL-SUMMARY.md with completion details
